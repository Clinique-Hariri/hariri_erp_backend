...
class CheckupAnalysisStatus
{
  ...
  public static function all($translated = false): array
  {
    return [
      ...
      self::IN_PROGRESS => $translated ? __('patients::app.in_progress') : self::IN_PROGRESS,
  ...
  public static function get_next_statuses(string $status): array
  {
    ...
    $map = [
      self::DRAFT => [PermissionNames::CHECKUP_SERVICES_UPDATE_TO_PENDING => self::get_resource(self::PENDING)],
      self::PENDING => [PermissionNames::CHECKUP_SERVICES_UPDATE_TO_IN_PROGRESS => self::get_resource(self::IN_PROGRESS)],
      self::IN_PROGRESS => [PermissionNames::CHECKUP_SERVICES_UPDATE_TO_COMPLETED => self::get_resource(self::COMPLETED)],
      self::COMPLETED => [],
    ];
