...
class UpdatePatientRequest extends FormRequest
{
  public static function rules($id): array
  {
    $rules = [
      ...
      'insurance_number' => ['nullable', 'string', 'max:255', 'unique:patients,insurance_number,' . $id],
      'insured_name' => ['nullable', 'string', 'max:255'],
      'whatsapp_number' => ['nullable', 'string', 'max:20', 'regex:/^(?:\+|00)?\d{7,15}$/', 'unique:patients,whatsapp_number,' . $id],
      'email' => ['nullable', 'email', 'max:255', 'unique:patients,email,' . $id],
      'avatar' => ['nullable', 'image', 'mimes:jpeg,png,jpg,gif,svg', 'max:10240'],
//      'status' => ['required', 'string', 'in:' . implode(',', PatientStatus::all())],
    ];
